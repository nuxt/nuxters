...
<template>
  <div class="relative w-full md:max-w-[400px] lg:max-w-[600px] min-h-[300px] md:min-h-[350px] lg:min-h-[222px]"
      ...
      <div v-else-if="linked['github']" class="w-full h-full">
        ...
        <div class="absolute right-2 top-2">
          <UButton class="transitions-colors duration-200" to="/logout" external size="xs" icon="i-ph-power"
            label="logout" color="gray" variant="ghost" />
        </div>
  ...
  "
        ...
        <div
          ...
          <div class="flex flex-col gap-y-4 justify-center w-full">
            <UAvatar :src="`https://avatars.githubusercontent.com/u/${contributor.githubId}`" size="2xl"
            ...
            <div class="flex items-center">
              ...
              <UButton variant="ghost" icon="i-ph-info" color="gray" @click.stop="isOpen = true"
                class="ml-1 transitions-color duration-200 z-50" aria-label="show score table" />
              <UModal class="relative" v-model="isOpen" :ui="{
                background: 'bg-gray-900',
                container: 'flex min-h-full md:items-center justify-center text-center',
                padding: 'p-0',
                overlay: { background: 'backdrop-blur bg-gray-800/70' },
              }">
                <UButton class="absolute right-2 top-2 transition-colors duration-200" color="white" variant="ghost"
                  size="xl" icon="i-ph-x" @click="isOpen = false" />
                <div class="flex flex-col justify-center gap-y-2 text-gray-300 text-lg">
                  <h5 class="text-2xl text-white font-medium px-4 py-3 pr-10 bg-gray-950">How is the score calculated?
                  </h5>
          ...
          <div class="flex flex-col gap-y-6 text-gray-300 w-full">
            <div class="flex items-center justify-between w-full">
              ...
              <UCheckbox :ui="{
                base: 'h-5 w-5',
              }" disabled v-model="hasMergedPullRequests" />
            </div>

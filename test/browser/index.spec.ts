...
test('directory filters, paginates and restores filters through browser history', async ({ page, goto }) => {
  ...
  await page.getByPlaceholder('Search countries', { exact: true }).fill('France')
  ...
  await page.goBack()
  await expect(page).toHaveURL(/page=2/)
  await expect(page.getByLabel('Country', { exact: true })).toContainText('All countries')
})
...
test('explorer rotates until interaction and supports trackpad pinch and avatar previews', async ({ page, goto }) => {
  ...
  await page.getByPlaceholder('Search countries', { exact: true }).fill('France')
  await page.getByRole('option', { name: 'France', exact: true }).click()
  await expect(globe.locator('[data-country="country-fr"] .people-globe__avatars img')).toHaveCount(3)
...
test('locating a person turns the globe without leaving the directory', async ({ page, goto }) => {
  await goto('/people', { waitUntil: 'hydration' })
  const globe = page.locator('.people-globe')
  await expect(globe).toHaveAttribute('data-ready', 'true')
  const before = await globe.getAttribute('data-rotation')
  const person = page.getByRole('button', { name: 'Locate 001123 in Vietnam', exact: true })
  await person.click()
  await expect(page).toHaveURL(/\/people$/)
  ...
  await page.keyboard.press('ArrowRight')
  await person.click()
  await expect.poll(async () => Math.abs(Number(await globe.getAttribute('data-rotation')) - focused)).toBeLessThan(0.001)
  await expect(page.getByRole('link', { name: 'Open 001123\'s profile', exact: true })).toHaveAttribute('href', '/001123')
})
